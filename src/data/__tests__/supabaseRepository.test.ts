...
describe('SupabaseRepository — writes carry the correct familyId', () => {
  ...
  it('upsertDog sends family_id mapped from dog.familyId', async () => {
    const { client, calls } = makeMockClient();
    const repo = new SupabaseRepository(client);
    const dog: Dog = { id: 'dog-1', familyId: 'fam-42', name: 'טופי', heroBackgroundId: 'park', walksPerDay: 4 };

    await repo.upsertDog(dog);

    expect(calls[0].table).toBe('dogs');
    expect((calls[0].payload as any).family_id).toBe('fam-42');
    expect((calls[0].payload as any).hero_background_id).toBe('park');
  });
...
describe('SupabaseRepository — reads map rows correctly (toDog/toRule/toEntry/toWalk)', () => {
  ...
  it('getDog maps a full row, including nullish photo_url/notes/sex -> undefined', async () => {
    ...
    expect(dog).toEqual({ id: 'dog-1', familyId: 'fam-42', name: 'טופי', photoUrl: undefined, heroBackgroundId: undefined, walksPerDay: 3, notes: undefined, sex: undefined });
  });
  ...
  it('getDog maps a full row with photo_url/notes/sex present', async () => {
      ...
      from: () => ({
        select: () => ({
          eq: () => ({
            maybeSingle: () =>
              Promise.resolve({
                data: { id: 'dog-1', family_id: 'fam-42', name: 'טופי', photo_url: 'https://x/y.png', hero_background_id: 'park', walks_per_day: 3, notes: 'אוהב לרוץ', sex: 'male' },
                error: null,
              }),
          }),
        }),
    ...
    expect(dog.heroBackgroundId).toBe('park');
  ...
  it('getDogs maps every row for the family via toDog (arbitrary N, not just one)', async () => {
    ...
    expect(dogs).toEqual([
      { id: 'dog-1', familyId: 'fam-42', name: 'טופי', photoUrl: undefined, heroBackgroundId: undefined, walksPerDay: 4, notes: undefined, sex: undefined },
      { id: 'dog-2', familyId: 'fam-42', name: 'ריקי', photoUrl: undefined, heroBackgroundId: undefined, walksPerDay: 2, notes: undefined, sex: undefined },
    ]);
  });
