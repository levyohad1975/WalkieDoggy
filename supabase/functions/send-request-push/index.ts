...
// recipientUserIds/title/body/kind/event — a modified client could have
...
// (plus an optional `kind` HINT purely to save one lookup — never trusted
// for authorization) — everything else is derived here, server-side, from:
//   1. The caller's identity, verified from their Authorization bearer
//      token against Supabase Auth (mirrors current_profile_id()'s
//      auth.uid()-based trust model already used throughout this schema —
...
//      time — the canonical, unit-tested copy lives there; keep both in
...
//      — queries through this client run AS the caller, under RLS, so
...
//      and callerIsAdmin (is_family_admin() RPC) — all via that
...
//      row by requestId (checked against both request tables — `kind` from
//      the client, if sent, only picks which table to try FIRST, it is
//      never trusted as authorization).
//   e. Run validateAndRoutePushEvent(row, event, ctx) — rejects outright if
...
//      filled in with server-loaded names/times — never client text. Any
...
//      (kind:requestId:event) via claim_request_push_event() — a repeat
...
// already-committed request DB action that triggered this call — by
...
// Inlined copy of src/logic/pushRouting.ts's validateAndRoutePushEvent() —
...
function templateFor(kind: PushRequestKind, event: PushRequestEvent, row: RequestRowForPush): { title: string; body: string } {
  ...
  if (event === 'approved') {
    return {
      ...
      body: row.proposedTime ? `שעת הטיול שונתה ל-${row.proposedTime}` : 'שעת הטיול שונתה כמבוקש',
    };
  }
  return { title: 'בקשת שינוי השעה שלך נדחתה', body: 'בקשת שינוי השעה שביקשת נדחתה' };
}
...
Deno.serve(async (req: Request) => {
  try {
    ...
    // User-scoped client: runs AS the caller under RLS — auth.uid()-based
    ...
    // ---- Step 3: parse the request body — ONLY a trigger hint, never authorization ----
    ...
    if (row.kind === 'timeChange' && event === 'created') {
      ...
      // identity), not by users.id (a family-member PROFILE) — users.
    ...
    if (!routing.authorized) {
      ...
      // request") — respond 200/ok:false so this never surfaces as if the
    ...
    // ---- Step 8: IDEMPOTENCY CLAIM — strictly AFTER authorization, never before ----
    ...
    // at all (verified by this ordering — see the final report for how
    ...
    // shape — see that file's doc comment on keeping both in sync).
    ...
    if (!claimed) {
      ...
      // is currently sending it and that attempt isn't stale yet — the
    ...
    // ---- Step 9: template the content server-side — never forward client text ----
    ...
    try {
      ...
      if (!tokenRows || tokenRows.length === 0) {
        await serviceClient.rpc('mark_request_push_event', { p_dedupe_key: dedupeKey, p_status: 'sent' });
        return new Response(JSON.stringify({ ok: true, sent: 0, reason: 'no active tokens' }), { status: 200 });
      }
      ...
      // Stale/invalid token handling (unchanged from before — a separate
      // concern from send-level idempotency, per Requirement 4): deactivate
      // a token Expo reports as permanently DeviceNotRegistered rather
      ...
      // The Expo call (or the token lookup) failed AFTER the key was
      // claimed — mark it 'failed' rather than leaving it stuck 'sending',
      // so a later legitimate retry doesn't have to wait out the full
      // staleness window for no reason.
      await serviceClient.rpc('mark_request_push_event', { p_dedupe_key: dedupeKey, p_status: 'failed' });
      return new Response(JSON.stringify({ ok: false, error: String(sendErr) }), { status: 200 });
    }
  } catch (err) {
