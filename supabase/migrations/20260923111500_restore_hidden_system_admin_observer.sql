-- Restore cross-family hidden observer access for the system administrator.
-- The observer remains read-only. The client hides all System Admin controls
-- while an observer session is active, so those controls never appear in a
-- viewed family's Settings screen.
